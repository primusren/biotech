...
.card-row:nth-child(even) { background: #f8fafc; }
.card-field { margin-bottom: 5px; line-height: 1.45; }
.card-field-label {
    font-weight: 700; font-size: 0.72rem; color: #1a365d;
    text-transform: uppercase; letter-spacing: 0.03em;
    display: block; margin-bottom: 1px;
}
.card-field-value { font-size: 0.82rem; color: #2d3748; word-wrap: break-word; }
...
def _is_wide_table(header, rows):
    """Heuristic: table needs card layout if it has many columns or long cell content."""
    if len(header) > 3:
        return True
    # Check average cell length
    total_len = sum(len(cell) for row in rows for cell in row)
    avg_len = total_len / max(len(rows) * len(header), 1)
    if avg_len > 40:
        return True
    return False


def _render_table_as_cards(header, rows):
    """Render a table as vertical cards (mobile-friendly)."""
    html_parts = ['<div class="card-table">']
    for row in rows:
        ...
        for col_idx, col_name in enumerate(header):
            ...
            html_parts.append(
                ...
                f'<span class="card-field-label">{col_name}</span>'
                f'<span class="card-field-value">{val}</span>'
                f'</div>'
            )
...
def _render_md_with_tables(md_text, max_table_rows=50):
    """Render markdown text, converting embedded tables to mobile-friendly layouts.

    Wide tables (many columns or long text) → vertical card layout.
    Compact tables (2-3 short columns) → st.dataframe.
    """
    if not md_text:
    ...
    while i < len(lines):
        ...
        if "|" in stripped and i + 1 < len(lines) and re.match(r"^\s*\|[\s\-:|]+\|\s*$", lines[i + 1].strip()):
            ...
            if header and rows:
                ...
                if _is_wide_table(header, clean_rows):
                    # Card layout for wide/text-heavy tables
                    _render_table_as_cards(header, clean_rows)
                else:
                    # Compact table — use dataframe
                    try:
